...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2) {
    ...
    base_path = "../tests/data/lidarply";
    LOG(WARNING)
        << "No lidar PLY dataset path specified; defaulting to the test "
           "directory.";
  } else {
    base_path = argv[1];
    LOG(INFO) << "Loading lidar files from " << base_path;
  }
  ...
  std::unique_ptr<LidarFuser> fuser =
      datasets::lidarply::createFuser(base_path, seq_id);
  if (!fuser) {
