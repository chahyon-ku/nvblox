...
float getRMSEClosestPointDistance(const Pointcloud& pointcloud_a,
  ...
  thrust::device_ptr<float> thrust_ptr(squared_distances.data());
  float total_squared_distance = thrust::reduce(
      thrust_ptr, thrust_ptr + num_points_a, 0.0f, thrust::plus<float>());
...
class LidarMotionCompensationTest : public ::testing::Test {
 protected:
  std::string data_base_path_ = "./data/lidarply/";
